...
func (ob *OrdianService) GetOrdianByIdPDF(id string) (*d.FormOrdianReq, error) {
...
func (ob *OrdianService) DownloadOrdianByID(id string) ([]byte, error) {
  ...
  <!DOCTYPE html>
	...
	renderedHTML, err := renderTemplate(tmpl, ordian)
	if err != nil {
		return nil, err
	}

	pdfBytes, err := convertHTMLToPDF(renderedHTML)
	if err != nil {
		return nil, err
	}
